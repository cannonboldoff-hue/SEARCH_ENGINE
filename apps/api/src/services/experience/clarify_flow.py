"""
Experience Card clarify flow: planner → guardrail → question writer / answer applier.

Architecture:
- Canonical normalizer: one card shape for all clarify logic.
- LLM planner: ask | autofill | stop + target_type/target_field/target_child_type.
- Backend guardrail: validate plan, fallback if invalid.
- Separate LLM: question writer (phrasing only), answer applier (patch only).
"""
