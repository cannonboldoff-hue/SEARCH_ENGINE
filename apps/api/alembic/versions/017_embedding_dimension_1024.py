"""Change embedding columns from vector(384) to vector(1024) for Jina v3.
...
def upgrade() -> None:
    # experience_cards: drop HNSW index, replace embedding column with vector(1024)
    op.execute("DROP INDEX IF EXISTS ix_experience_cards_embedding_hnsw")
    op.execute("ALTER TABLE experience_cards ADD COLUMN embedding_new vector(1024)")
    ...
    op.execute("ALTER TABLE experience_card_children ADD COLUMN embedding_new vector(1024)")
...
def downgrade() -> None:
    op.execute("DROP INDEX IF EXISTS ix_experience_card_children_embedding_hnsw")
    op.execute("ALTER TABLE experience_card_children ADD COLUMN embedding_old vector(384)")
    op.execute("ALTER TABLE experience_card_children DROP COLUMN embedding")
    ...
    op.execute("DROP INDEX IF EXISTS ix_experience_cards_embedding_hnsw")
    op.execute("ALTER TABLE experience_cards ADD COLUMN embedding_old vector(384)")
    op.execute("ALTER TABLE experience_cards DROP COLUMN embedding")
